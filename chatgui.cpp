    ...
    : wxScrolledWindow(parent, id)
{
    ...
    //_chatLogic = new std::unique_ptr<ChatLogic>;
	//std::unique_ptr<int> u3(new int);
	//std::unique_ptr<int> unique;
	//unique = (new int); // create a unique pointer on the stack
	std::unique_ptr<ChatLogic> _chatLogic(new ChatLogic);
	//std::unique_ptr<ChatLogic> unique = std::make_unique<ChatLogic>(new A);
